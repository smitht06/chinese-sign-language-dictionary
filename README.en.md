...
| `scripts/build_asl_videos.py` | Standalone tool: transcodes ASL-LEX videos to H.264 and matches them to signs by English word (not used by the app) |
...
>
> Note: the app copies the bundled database into local storage on **first launch only** — after data changes, uninstall and reinstall the app for them to take effect.
...
### ASL sign videos (standalone tool, not used by the app)

`scripts/build_asl_videos.py` is kept as a standalone tool: it transcodes [ASL-LEX](https://asl-lex.org/) `.webm` videos to H.264 `.mp4` and matches them to signs in `sign_themed.db` by English word, producing `build/asl_videos.json` and the audit log `build/asl_videos_match.log`. In-app video playback was built then removed by user preference (ASL pictures instead of videos); the script remains for future use.

```bash
python3 scripts/build_asl_videos.py   # requires ffmpeg + the ASL Data/ dataset
```
